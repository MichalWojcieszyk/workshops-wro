...
RSpec.describe PaymentsController do
  ...
  describe 'PUT #update' do
  	let!(:payment) { create :payment, student: student, sum: 100, date: Date.today }
  	let(:sum) { 200 }
  	let(:date) { Date.today + 1}
  	let!(:params) do
  		{ id: payment.id, payment: { sum: sum, date: date } }
  	end
  	...
  	context 'success' do
  		...
  		context 'updates payment' do
  			subject { -> { put :update, params } }
  			it { is_expected.to change{ payment.reload.sum }.to(sum) }
  			it { is_expected.to change{ payment.reload.date }.to(date) }
  		end
  	end
